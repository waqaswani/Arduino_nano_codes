...
void setup() {
 ...
 pinMode(13,INPUT);
 pinMode(A7,INPUT);//Pin A7 is connected to LM35 a temperature sensor
 pinMode(A0,INPUT);//Pin A0 is connected to LDR
...
void loop() {
  ...
  lcd.setCursor(0, 0);
   ...
   temp1=analogRead(A7);
    temp2=((temp1*4.88)/10);	//Reading value from lm35 10mv per degree celsius
// 5 volts corresponds to 1023 so accordingly we calculate
  ...
  for(int i=0;i<4;i++)
  {
    ...
    if(j!=5){//Indicating that one of the keys has been pressed
      ...
      if(count==4)
      {
        ...
        if(b==password)
          {lcd.clear();
    lcd.print(temp2);//Print temperature
    lcd.setCursor(0,1);//goto the next line(row 1 column 0)

    lcd.print(val);//Print light Intensity
