...
char key[4][4] = {{'1', '2', '3', '+'}, {'4', '5', '6', '-'}, {'7', '8', '9', '*'}, {'c', '0', '=', '/'}}; // a 4*4 matrix where every key of the matrix is mentioned
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    { if ((key[j][i] == '+') || (key[j][i] == '-') || (key[j][i] == '*') || (key[j][i] == '/')) //Checking for operator press
      ...
      { Serial.print('=');
        switch (op)
        {
          case '+': Serial.println(ctoi(num1, num1_count) + ctoi(num2, count));
            break;
          case '-': Serial.println(ctoi(num1, num1_count) - ctoi(num2, count));
            break;
          case '*': Serial.println(ctoi(num1, num1_count)*ctoi(num2, count));
          ...
          case '/':
            ...
            {
              Serial.print("inf"); //division by zero is not defined
            }
            else
              Serial.println(ctoi(num1, num1_count) / ctoi(num2, count));
            break;
          default: Serial.print(ctoi(num1, num1_count));
        ...
        {
          num1[count] = key[j][i];
          Serial.print(key[j][i]);//storing the first entered number
          count++;
          if (count >= 7)
          {
...
int row_read()
{
  ...
  if (digitalRead(row[0]))
  {
    ...
    while (digitalRead(row[0]));//wait until next key is pressed
    return 0;
  }
